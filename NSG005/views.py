...
def nsg005_page(request):
    dataset = {
        'title': 'TB-Lung-Visum-NSG005',
        'species': 'Human',
        'tissue_type': 'Lung',
        'granuloma_description': 'Medium-Large Granuloma',
        'age': 49,
        'sex': 'M',
        'bacteria': 'M. kansasii',
        'slide_number': 'V53F21-058',
        'sample_id': 'H1219915',
        'pubid': 'NSG005',
        'method': 'Visium',
        'default_img': f"/static/images/spots/H1219915.jpeg"
    }
    return render(request, 'dataview/view.html', {'dataset': dataset})

@csrf_exempt
def get_gene_list(request):
    adata = load_adata("NSG005")
...
def plot_gene_image(request):
    if request.method == "POST":
        try:
            ...
            output_dir = os.path.join("frontend", "static", "generated", "H1219915")
            ...
            plt.ioff()
            fig = sc.pl.spatial(
                adata, color=[gene], library_id="H1219915",
                show=False, alpha=0.75, alpha_img=0.3, cmap="turbo", return_fig=True
            )
            fig.savefig(img_path, dpi=150)
            plt.close(fig)

            return JsonResponse({"image_url": f"/static/generated/H1219915/{gene}.png"})
