...
def acpmb010_page(request):
    dataset = {
        'title': 'TB-Lung-Visum-ACPMB010',
        'species': 'Human',
        'tissue_type': 'Lung',
        ...
        'slide_number': 'V53F21-129',
        'sample_id': 'ACPMB10',
        'pubid': 'ACPMB010',
        'method': 'Visium',
        'default_img': f"/static/images/spots/ACPMB10.jpeg"
    }
    return render(request, 'dataview/view.html', {'dataset': dataset})

@csrf_exempt
def get_gene_list(request):
    adata = load_adata("ACPMB010")
...
def plot_gene_image(request):
    if request.method == "POST":
        try:
            ...
            output_dir = os.path.join("frontend", "static", "generated", "ACPMB10")
            ...
            plt.ioff()
            fig = sc.pl.spatial(
                adata, color=[gene], library_id="ACPMB10",
                show=False, alpha=0.75, alpha_img=0.3, cmap="turbo", return_fig=True
            )
            fig.savefig(img_path, dpi=150)
            plt.close(fig)

            return JsonResponse({"image_url": f"/static/generated/ACPMB10/{gene}.png"})
