...
def ytb008_page(request):
    dataset = {
        'title': 'TB-Lung-Visum-YTB008',
        'species': 'Human',
        'tissue_type': 'Lung',
        'granuloma_description': 'Smaller no-border Granuloma',
        'age': 59,
        'sex': 'F',
        'bacteria': 'M. tuberculosis complex',
        'slide_number': 'V53F21-042',
        'sample_id': 'YTB008',
        'pubid': 'YTB008',
        'method': 'Visium',
        'default_img': f"/static/images/spots/YTB008.jpeg"
    }
    return render(request, 'dataview/view.html', {'dataset': dataset})

@csrf_exempt
def get_gene_list(request):
    adata = load_adata("YTB008")
...
def plot_gene_image(request):
    if request.method == "POST":
        try:
            ...
            output_dir = os.path.join("frontend", "static", "generated", "YTB008")
            ...
            plt.ioff()
            fig = sc.pl.spatial(
                adata, color=[gene], library_id="YTB008",
                show=False, alpha=0.75, alpha_img=0.3, cmap="turbo", return_fig=True
            )
            fig.savefig(img_path, dpi=150)
            plt.close(fig)

            return JsonResponse({"image_url": f"/static/generated/YTB008/{gene}.png"})
