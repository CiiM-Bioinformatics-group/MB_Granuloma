...
def nsg019_page(request):
    dataset = {
        'title': 'TB-Lymphnode-Visum-NSG019',
        'species': 'Human',
        'tissue_type': 'Lymphnode',
        'granuloma_description': 'Giant cells, no well-formed granuloma',
        'age': 13,
        'sex': 'F',
        'bacteria': 'M. tuberculosis',
        'slide_number': 'V53F21-047',
        'sample_id': 'H2311362',
        'pubid': 'NSG019',
        'method': 'Visium',
        'default_img': f"/static/images/spots/H2311362.jpeg"
    }
    return render(request, 'dataview/view.html', {'dataset': dataset})

@csrf_exempt
def get_gene_list(request):
    adata = load_adata("NSG019")
...
def plot_gene_image(request):
    if request.method == "POST":
        try:
            ...
            output_dir = os.path.join("frontend", "static", "generated", "H2311362")
            ...
            plt.ioff()
            fig = sc.pl.spatial(
                adata, color=[gene], library_id="H2311362",
                show=False, alpha=0.75, alpha_img=0.3, cmap="turbo", return_fig=True
            )
            fig.savefig(img_path, dpi=150)
            plt.close(fig)

            return JsonResponse({"image_url": f"/static/generated/H2311362/{gene}.png"})
