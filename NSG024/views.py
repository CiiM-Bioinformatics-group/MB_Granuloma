...
def nsg024_page(request):
    dataset = {
        'title': 'TB-Lung-Visum-NSG024',
        'species': 'Human',
        'tissue_type': 'Lung',
...
def plot_gene_image(request):
    if request.method == "POST":
        try:
            ...
            output_dir = os.path.join("frontend", "static", "generated", "H2317069")
            ...
            plt.ioff()
            fig = sc.pl.spatial(
                adata, color=[gene], library_id="H2317069",
                show=False, alpha=0.75, alpha_img=0.3, cmap="turbo", return_fig=True
            )
            fig.savefig(img_path, dpi=150)
            plt.close(fig)

            return JsonResponse({"image_url": f"/static/generated/H2317069/{gene}.png"})
