...
def ytb001_page(request):
    dataset = {
        'title': 'TB-Lung-Visum-YTB001',
        'species': 'Human',
        'tissue_type': 'Lung',
        'granuloma_description': 'Small',
        'age': 81,
        'sex': 'M',
        'bacteria': 'M. tuberculosis complex',
        'slide_number': 'V53F21-095',
        'sample_id': 'YTB001',
        'pubid': 'YTB001',
        'method': 'Visium',
        'default_img': f"/static/images/spots/YTB001.jpeg"
    }
    return render(request, 'dataview/view.html', {'dataset': dataset})

@csrf_exempt
def get_gene_list(request):
    adata = load_adata("YTB001")
...
def plot_gene_image(request):
    if request.method == "POST":
        try:
            ...
            output_dir = os.path.join("frontend", "static", "generated", "YTB001")
            ...
            plt.ioff()
            fig = sc.pl.spatial(
                adata, color=[gene], library_id="YTB001",
                show=False, alpha=0.75, alpha_img=0.3, cmap="turbo", return_fig=True
            )
            fig.savefig(img_path, dpi=150)
            plt.close(fig)

            return JsonResponse({"image_url": f"/static/generated/YTB001/{gene}.png"})
