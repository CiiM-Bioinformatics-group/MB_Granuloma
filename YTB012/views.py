...
def ytb012_page(request):
    dataset = {
        'title': 'TB-Lung-Visum-YTB012',
        'species': 'Human',
        'tissue_type': 'Lung',
        'granuloma_description': 'Very small Granuloma',
        'age': 79,
        'sex': 'F',
...
def plot_gene_image(request):
    if request.method == "POST":
        try:
            ...
            output_dir = os.path.join("frontend", "static", "generated", "YTB012")
            ...
            plt.ioff()
            fig = sc.pl.spatial(
                adata, color=[gene], library_id="YTB012",
                show=False, alpha=0.75, alpha_img=0.3, cmap="turbo", return_fig=True
            )
            fig.savefig(img_path, dpi=150)
            plt.close(fig)

            return JsonResponse({"image_url": f"/static/generated/YTB012/{gene}.png"})
