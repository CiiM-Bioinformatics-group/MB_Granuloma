...
def acpmb009_page(request):
    dataset = {
        'title': 'TB-Lung-Visum-ACPMB009',
        'species': 'Human',
        'tissue_type': 'Lung',
        'granuloma_description': 'Large Granuloma, lots of B-clusters',
        'age': 58,
        'sex': 'F',
        'bacteria': 'M. tuberculosis complex',
        'slide_number': 'V53F21-071',
        'sample_id': 'ACPMB9',
        'pubid': 'ACPMB009',
        'method': 'Visium',
        'default_img': f"/static/images/spots/ACPMB9.jpeg"
    }
    return render(request, 'dataview/view.html', {'dataset': dataset})

@csrf_exempt
def get_gene_list(request):
    adata = load_adata("ACPMB009")
...
def plot_gene_image(request):
    if request.method == "POST":
        try:
            ...
            output_dir = os.path.join("frontend", "static", "generated", "ACPMB9")
            ...
            plt.ioff()
            fig = sc.pl.spatial(
                adata, color=[gene], library_id="ACPMB9",
                show=False, alpha=0.75, alpha_img=0.3, cmap="turbo", return_fig=True
            )
            fig.savefig(img_path, dpi=150)
            plt.close(fig)

            return JsonResponse({"image_url": f"/static/generated/ACPMB9/{gene}.png"})
