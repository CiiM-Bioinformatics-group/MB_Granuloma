...
def ytb009_page(request):
    dataset = {
        'title': 'TB-Lymphnode-Visum-YTB009',
        'species': 'Human',
        'tissue_type': 'Lymphnode',
        'granuloma_description': 'Lots of small, one Large Granuloma',
        'age': 64,
        'sex': 'F',
...
def plot_gene_image(request):
    if request.method == "POST":
        try:
            ...
            output_dir = os.path.join("frontend", "static", "generated", "YTB009")
            ...
            plt.ioff()
            fig = sc.pl.spatial(
                adata, color=[gene], library_id="YTB009",
                show=False, alpha=0.75, alpha_img=0.3, cmap="turbo", return_fig=True
            )
            fig.savefig(img_path, dpi=150)
            plt.close(fig)

            return JsonResponse({"image_url": f"/static/generated/YTB009/{gene}.png"})
